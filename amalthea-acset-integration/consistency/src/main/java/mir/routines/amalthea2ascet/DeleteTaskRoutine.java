...
public class DeleteTaskRoutine extends AbstractRoutine {
  ...
  private static class Update extends AbstractRoutine.Update {
    ...
    public void updateModels(final Task task, final tools.vitruv.methodologisttemplate.model.model.Task ascettask, @Extension final Amalthea2ascetRoutinesFacade _routinesFacade) {
      if ((ascettask != null)) {
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    retrievedValues = new mir.routines.amalthea2ascet.DeleteTaskRoutine.Match(getExecutionState()).match(inputValues.task);
    if (retrievedValues == null) {
    	return false;
    }
    // This execution step is empty
    new mir.routines.amalthea2ascet.DeleteTaskRoutine.Update(getExecutionState()).updateModels(inputValues.task, retrievedValues.ascettask, getRoutinesFacade());
    return true;
  }
}
