...
public class CreateAscetTaskRoutine extends AbstractRoutine {
    ...
    public void updateModels(final Task task, final ComponentContainer container, @Extension final Amalthea2ascetRoutinesFacade _routinesFacade) {
      ...
      if ((selected != null)) {
        ...
        try {
          ...
          {
            InputOutput.<String>println("[Reaction] Attempting to call SymbolicComparison.symbolicVitruviusChoice");
            InputOutput.<String>println(("  - selected = " + selected));
            final Class<?> symbolicClass = Class.forName("edu.neu.ccs.prl.galette.concolic.knarr.runtime.SymbolicComparison");
            InputOutput.<String>println(("  - Found class: " + symbolicClass));
            final Method method = symbolicClass.getMethod("symbolicVitruviusChoice", Integer.class, Integer.TYPE, Integer.TYPE);
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    // This execution step is empty
    new mir.routines.amalthea2ascet.CreateAscetTaskRoutine.Update(getExecutionState()).updateModels(inputValues.task, inputValues.container, getRoutinesFacade());
    return true;
  }
}
