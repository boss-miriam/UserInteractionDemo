...
public class TaskDeletedReaction extends AbstractReaction {
  ...
  public boolean isCurrentChangeMatchingTrigger(final EChange change) {
    ...
    DeleteEObject<tools.vitruv.methodologisttemplate.model.model2.Task> _localTypedChange = (DeleteEObject<tools.vitruv.methodologisttemplate.model.model2.Task>) change;
  ...
  public void executeReaction(final EChange change, final ReactionExecutionState executionState, final RoutinesFacade routinesFacadeUntyped) {
    ...
    tools.vitruv.methodologisttemplate.model.model2.Task affectedEObject = (tools.vitruv.methodologisttemplate.model.model2.Task)deleteChange.getAffectedElement();
    if (getLogger().isTraceEnabled()) {
    	getLogger().trace("Passed complete precondition check of Reaction " + this.getClass().getName());
    }
    
    new mir.reactions.amalthea2ascet.TaskDeletedReaction.Call(executionState).updateModels(deleteChange, affectedEObject, routinesFacade);
  }
}
