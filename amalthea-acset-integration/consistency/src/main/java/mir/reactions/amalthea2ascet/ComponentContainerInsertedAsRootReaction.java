...
public class ComponentContainerInsertedAsRootReaction extends AbstractReaction {
  ...
  public boolean isCurrentChangeMatchingTrigger(final EChange change) {
    ...
    InsertRootEObject<tools.vitruv.methodologisttemplate.model.model2.ComponentContainer> _localTypedChange = (InsertRootEObject<tools.vitruv.methodologisttemplate.model.model2.ComponentContainer>) change;
    if (!(_localTypedChange.getNewValue() instanceof tools.vitruv.methodologisttemplate.model.model2.ComponentContainer)) {
    	return false;
    }
    this.insertChange = (InsertRootEObject<tools.vitruv.methodologisttemplate.model.model2.ComponentContainer>) change;
    return true;
  }
  ...
  public void executeReaction(final EChange change, final ReactionExecutionState executionState, final RoutinesFacade routinesFacadeUntyped) {
    ...
    tools.vitruv.methodologisttemplate.model.model2.ComponentContainer newValue = (tools.vitruv.methodologisttemplate.model.model2.ComponentContainer)insertChange.getNewValue();
    int index = insertChange.getIndex();
    if (getLogger().isTraceEnabled()) {
    	getLogger().trace("Passed complete precondition check of Reaction " + this.getClass().getName());
    }
    
    new mir.reactions.amalthea2ascet.ComponentContainerInsertedAsRootReaction.Call(executionState).updateModels(insertChange, newValue, index, routinesFacade);
  }
}
