...
public class CreateInitTaskRoutine extends AbstractRoutine {
  ...
  private static class Match extends AbstractRoutine.Match {
    ...
    public RetrievedValues match(final Task task, final ComponentContainer container) throws IOException {
      ...
      if (hasCorrespondingElements(
          ...
          ecore.tools.vitruv.methodologisttemplate.model.ascet.InitTask.class,
      	null, // correspondence precondition checker
      	null
      )) {
      	return null;
      }
      return new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateInitTaskRoutine.Match.RetrievedValues(AscetModule);
    }
  }
  ...
  private static class Create extends AbstractRoutine.Create {
    ...
    public CreatedValues createElements() {
      ecore.tools.vitruv.methodologisttemplate.model.ascet.InitTask initTask = createObject(() -> {
      	return ecore.tools.vitruv.methodologisttemplate.model.ascet.impl.AscetFactoryImpl.eINSTANCE.createInitTask();
      });
      return new CreatedValues(initTask);
    }
  }
  ...
  private static class Update extends AbstractRoutine.Update {
    ...
    public void updateModels(final Task task, final ComponentContainer container, final AscetModule AscetModule, final InitTask initTask, @Extension final Amalthea2ascetRoutinesFacade _routinesFacade) {
  ...
  public CreateInitTaskRoutine(final Amalthea2ascetRoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final Task task, final ComponentContainer container) {
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    retrievedValues = new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateInitTaskRoutine.Match(getExecutionState()).match(inputValues.task, inputValues.container);
    if (retrievedValues == null) {
    	return false;
    }
    createdValues = new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateInitTaskRoutine.Create(getExecutionState()).createElements();
    new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateInitTaskRoutine.Update(getExecutionState()).updateModels(inputValues.task, inputValues.container, retrievedValues.AscetModule, createdValues.initTask, getRoutinesFacade());
    return true;
  }
}
