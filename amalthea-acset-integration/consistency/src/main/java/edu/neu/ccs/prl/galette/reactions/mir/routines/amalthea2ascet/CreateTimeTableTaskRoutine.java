...
public class CreateTimeTableTaskRoutine extends AbstractRoutine {
  ...
  private static class Match extends AbstractRoutine.Match {
    ...
    public RetrievedValues match(final Task task, final ComponentContainer container) throws IOException {
      ...
      if (hasCorrespondingElements(
          ...
          ecore.tools.vitruv.methodologisttemplate.model.ascet.TimeTableTask.class,
      	null, // correspondence precondition checker
      	null
      )) {
      	return null;
      }
      return new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateTimeTableTaskRoutine.Match.RetrievedValues(AscetModule);
    }
  }
  ...
  private static class Create extends AbstractRoutine.Create {
    ...
    public CreatedValues createElements() {
      ecore.tools.vitruv.methodologisttemplate.model.ascet.TimeTableTask timeTableTask = createObject(() -> {
      	return ecore.tools.vitruv.methodologisttemplate.model.ascet.impl.AscetFactoryImpl.eINSTANCE.createTimeTableTask();
      });
      return new CreatedValues(timeTableTask);
    }
  }
  ...
  private static class Update extends AbstractRoutine.Update {
    ...
    public void updateModels(final Task task, final ComponentContainer container, final AscetModule AscetModule, final TimeTableTask timeTableTask, @Extension final Amalthea2ascetRoutinesFacade _routinesFacade) {
  ...
  public CreateTimeTableTaskRoutine(final Amalthea2ascetRoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final Task task, final ComponentContainer container) {
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    retrievedValues = new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateTimeTableTaskRoutine.Match(getExecutionState()).match(inputValues.task, inputValues.container);
    if (retrievedValues == null) {
    	return false;
    }
    createdValues = new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateTimeTableTaskRoutine.Create(getExecutionState()).createElements();
    new edu.neu.ccs.prl.galette.reactions.mir.routines.amalthea2ascet.CreateTimeTableTaskRoutine.Update(getExecutionState()).updateModels(inputValues.task, inputValues.container, retrievedValues.AscetModule, createdValues.timeTableTask, getRoutinesFacade());
    return true;
  }
}
